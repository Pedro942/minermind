...
// minermind: optional setting to unlock wallet for block minting only;
...
// minermind: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // minermind: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // minermind: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // minermind: not avatar mode
                        {
...
// minermind: create coin stake transaction
...
// minermind: check 'spent' consistency between wallet and txindex
// minermind: fix wallet spent state according to txindex
...
// minermind: disable transaction (only for coinstake)
